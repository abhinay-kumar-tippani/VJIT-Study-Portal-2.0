 ...
 * eval/generate.ts
 *
 * Auto-generates eval/questions.json from ingested chunks.
 *
 * Requirements:
 * 1. Stratified sampling: 15 chunks per subject (IAI, PC, OOPs-Java, DBMS) with text.length > 800.
 * 2. File-level cap: MAX 2 questions per source file (expectedDriveFileId) to ensure broad document coverage.
 * 3. Atomic checkpointing & resume: APPENDS to eval/questions.json after EVERY question generated.
 *    On restart, skips chunks whose sourceChunkId is already present.
 * 4. Deduplication: Drops generated questions with cosine similarity > 0.9 against existing questions.
 *
 * Run:
 *   npx tsx eval/generate.ts
 */
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════╗');
  console.log('║   VJIT RAG — Stratified Question Dataset Generator   ║');
  console.log('║   Per-File Cap: Max 2 Qs / File | Auto-Checkpoint   ║');
  console.log('╚══════════════════════════════════════════════════════╝\n');
  ...
  const outPath = path.resolve(process.cwd(), 'eval', 'questions.json');
  ...
  if (fs.existsSync(outPath)) {
    try {
      ...
      console.log(`📋 Found existing checkpoint: ${existingQuestions.length} questions loaded from ${outPath}`);
    } catch {
      existingQuestions = [];
    }
  }
  ...
  for (const subject of TARGET_SUBJECTS) {
    ...
    console.log(`━━━ 🎯 Sampling chunks for ${subject} (Need ${CHUNKS_PER_SUBJECT - currentSubjectCount} more) ━━━`);

    const candidates = await Chunk.aggregate([
      { $match: { subject, $expr: { $gt: [{ $strLenCP: '$text' }, 800] } } },
      { $sample: { size: 100 } },
    ]);
    ...
    console.log(`  Selected ${sampledChunks.length} chunks across distinct files (capped at ${MAX_QUESTIONS_PER_FILE}/file)`);
  ...
  saveCheckpoint();
  console.log(`\n💾 Completed! ${existingQuestions.length} total questions saved to ${outPath}\n`);
  process.exit(0);
}

main().catch((err) => {
  console.error('❌ eval:gen failed:', err);
  process.exit(1);
});
